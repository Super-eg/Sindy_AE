"""Generate Lorenz data by projecting all 3 state variables to high-dim space
using Legendre polynomial modes, and save to .npz.

The 3D Lorenz state (z_x, z_y, z_z) is embedded into an n_points-dimensional
observation via:

    x[t] = P0 * z_x + P1 * z_y + P2 * z_z
           + P3 * z_x^3 + P4 * z_y^3 + P5 * z_z^3   (nonlinear=True)

where P_i = P_i(y_spatial) are Legendre polynomials evaluated on a spatial
grid of n_points points in [-1, 1].  Analytic first and second derivatives
(dx, ddx) are computed from the Lorenz vector field without finite differences.

Run from examples/lorenz/:
    python3 generate_lorenz.py

Output .npz is passed to examples/train.py via --data <path>.
"""
import argparse
import os
import sys

_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT = os.path.abspath(os.path.join(_THIS_DIR, "..", ".."))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

import numpy as np

from example_lorenz import generate_lorenz_data


def get_split(n_ics, t, input_dim, linear, normalization, noise_strength, rng):
    """Simulate n_ics trajectories and return flat (n_ics*n_steps, input_dim) arrays."""
    ic_means  = np.array([0.0,  0.0, 25.0])
    ic_widths = 2 * np.array([36.0, 48.0, 41.0])
    ics = ic_widths * (rng.rand(n_ics, 3) - 0.5) + ic_means

    data = generate_lorenz_data(ics, t, input_dim, linear=linear,
                                normalization=normalization)

    n_steps = t.size
    x   = data['x'].reshape(-1, input_dim).astype(np.float32)
    dx  = data['dx'].reshape(-1, input_dim).astype(np.float32)
    ddx = data['ddx'].reshape(-1, input_dim).astype(np.float32)
    # data['z'] is already normalized (shape [n_ics, n_steps, 3])
    z   = data['z'].reshape(-1, 3).astype(np.float32)

    if noise_strength > 0:
        noise = (noise_strength * rng.randn(*x.shape)).astype(np.float32)
        x   += noise
        dx  += (noise_strength * rng.randn(*dx.shape)).astype(np.float32)
        ddx += (noise_strength * rng.randn(*ddx.shape)).astype(np.float32)

    return x, dx, ddx, z, data['sindy_coefficients'], data['modes'], data['y_spatial']


def main():
    parser = argparse.ArgumentParser(
        description="Generate Lorenz data via Legendre projection and save to .npz"
    )
    parser.add_argument("--n_train_ics",    type=int,   default=1024)
    parser.add_argument("--n_val_ics",      type=int,   default=20)
    parser.add_argument("--n_test_ics",     type=int,   default=20)
    parser.add_argument("--noise_strength", type=float, default=1e-6)
    parser.add_argument("--seed",           type=int,   default=0)
    parser.add_argument("--t_start",        type=float, default=0.0)
    parser.add_argument("--t_end",          type=float, default=5.0)
    parser.add_argument("--dt",             type=float, default=0.02)
    parser.add_argument("--input_dim",      type=int,   default=128,
                        help="Observation dimension (Legendre grid size, default: 128)")
    parser.add_argument("--linear",         action="store_true",
                        help="Use linear-only projection (default: nonlinear cubic terms)")
    parser.add_argument(
        "--out", default=None,
        help="Output .npz file path (default: legendre_full.npz)",
    )
    args = parser.parse_args()

    if args.out is None:
        mode = "linear" if args.linear else "full"
        args.out = f"legendre_{mode}.npz"

    rng = np.random.RandomState(args.seed)

    t             = np.arange(args.t_start, args.t_end, args.dt)
    normalization = np.array([1 / 40, 1 / 40, 1 / 40])
    linear        = args.linear

    print("=" * 60)
    print("Lorenz data generation — Legendre polynomial projection")
    print(f"  Input dim (n_points) : {args.input_dim}")
    print(f"  Projection           : {'linear (P0-P2)' if linear else 'nonlinear (P0-P5, cubic)'}")
    print(f"  Time range           : {args.t_start} → {args.t_end}  (dt={args.dt})")
    print(f"  Steps per trajectory : {t.size}")
    print(f"  Train ICs            : {args.n_train_ics}")
    print(f"  Val ICs              : {args.n_val_ics}")
    print(f"  Test ICs             : {args.n_test_ics}")
    print(f"  Noise                : {args.noise_strength}")
    print(f"  Output               : {args.out}")
    print("=" * 60)

    print(f"Generating training data ({args.n_train_ics} ICs) …")
    train_x, train_dx, train_ddx, train_z, sindy_coefficients, modes, y_spatial = get_split(
        args.n_train_ics, t, args.input_dim, linear, normalization,
        args.noise_strength, rng,
    )
    print(f"  → train_x shape: {train_x.shape}")

    print(f"Generating validation data ({args.n_val_ics} ICs) …")
    val_x, val_dx, val_ddx, val_z, _, _, _ = get_split(
        args.n_val_ics, t, args.input_dim, linear, normalization,
        args.noise_strength, rng,
    )
    print(f"  → val_x shape: {val_x.shape}")

    print(f"Generating test data ({args.n_test_ics} ICs) …")
    test_x, test_dx, test_ddx, test_z, _, _, _ = get_split(
        args.n_test_ics, t, args.input_dim, linear, normalization,
        args.noise_strength, rng,
    )
    print(f"  → test_x shape: {test_x.shape}")

    out_dir = os.path.dirname(os.path.abspath(args.out))
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)

    np.savez(
        args.out,
        # ── data splits ──────────────────────────────────────────────
        train_x=train_x,   train_dx=train_dx,   train_ddx=train_ddx,   train_z=train_z,
        val_x=val_x,       val_dx=val_dx,       val_ddx=val_ddx,       val_z=val_z,
        test_x=test_x,     test_dx=test_dx,     test_ddx=test_ddx,     test_z=test_z,
        # ── Lorenz metadata ───────────────────────────────────────────
        train_t=t,         val_t=t,             test_t=t,
        sindy_coefficients=sindy_coefficients,
        modes=modes,
        y_spatial=y_spatial,
        normalization=normalization,
        # ── scalar metadata ───────────────────────────────────────────
        t_start=np.float64(args.t_start),
        t_end=np.float64(args.t_end),
        dt=np.float64(args.dt),
        input_dim=np.int64(args.input_dim),
        noise_strength=np.float64(args.noise_strength),
        n_train_ics=np.int64(args.n_train_ics),
        n_val_ics=np.int64(args.n_val_ics),
        n_test_ics=np.int64(args.n_test_ics),
        linear=np.bool_(linear),
    )
    print(f"\nSaved to {args.out}")


if __name__ == '__main__':
    main()
