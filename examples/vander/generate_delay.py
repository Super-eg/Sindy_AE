...
def main():
    ...
    t_per_ic_start = float(t[min_idx])
    t_per_ic_end   = float(t[-1])
    print(f"  train          : {args.n_train_ics} ICs  ×  {t_per_ic_start:.2f} s ~ {t_per_ic_end:.2f} s  ({args.n_train_ics * n_valid:,} samples)")
    print(f"  val            : {args.n_val_ics} ICs    ×  {t_per_ic_start:.2f} s ~ {t_per_ic_end:.2f} s  ({args.n_val_ics * n_valid:,} samples)")
    print(f"  test           : {args.n_test_ics} ICs   ×  {t_per_ic_start:.2f} s ~ {t_per_ic_end:.2f} s  ({args.n_test_ics * n_valid:,} samples)")
